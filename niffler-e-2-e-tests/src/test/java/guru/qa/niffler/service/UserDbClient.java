...
public final class UserDbClient implements UserClient {
    ...
    public @Nonnull UserJson createUser(String username, String password) {
        return xaTransactionTemplate.execute(() -> {
            ...
            return UserJson.fromEntity(userdataUserRepository.create(userEntityRequiredField(username)), null);
        });
    }
        ...
        if (count > 0) {
            for (int i = 0; i < count; i++) {
                xaTransactionTemplate.execute(() -> {
                    ...
                    users.add(UserJson.fromEntity(adressee, FriendshipStatus.INVITE_RECEIVED));
                    return null;
                });
            }
        }
        ...
        if (count > 0) {
            for (int i = 0; i < count; i++) {
                xaTransactionTemplate.execute(() -> {
                    ...
                    users.add(UserJson.fromEntity(adressee, FriendshipStatus.FRIEND));
                    return null;
                });
            }
        }
